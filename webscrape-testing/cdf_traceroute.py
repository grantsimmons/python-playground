...
with open("trace_mfa.go.th.txt", 'r') as infile:
    i = 0
    ...
    with open("mfa.go.th.csv", 'w') as outfile:
...
data = pd.read_csv('mfa.go.th.csv', parse_dates=['date'], date_parser=mydateparser)
data.head()
